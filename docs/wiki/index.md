...
- [[net-highid-and-port-forwarding]] - HighID mechanics + the VPN question (a VPN REPLACES padMule's UPnP path, so it all hinges on provider port forwarding - and our hardcoded 4662 blocks it until port override ships). HighID validated on the dev box (2026-07-14, 5-link manual chain) AND on the iPad via unicast-SSDP UPnP (2026-07-17); topology since 2026-07-17 is XB8-bridged -> TP-Link BE9700 (real UPnP IGD), which replaced the manual chain.
