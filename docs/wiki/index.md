...
- **Kad maintenance (2026-08-05, [[build-progress]] 8cd):** padMule had NONE - no bucket refresh, no random lookup, no contact ping - so the routing table was fed only by the bootstrap and by accident. That was both the low contact count AND the thin keyword-search results, since search quality tracks table breadth. `maintain_kad` now runs a random-target lookup every 120s, and resume no longer re-seeds from disk alone (it was discarding the live table `pause()` had just folded into memory: 138 contacts became 21 in one background round trip).
