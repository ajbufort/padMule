...
| 6a | Kad128 (CUInt128) + nodes.dat + routing table | (implemented directly) | STARTED + real-fixture-validated. mule_proto::Kad128 (XOR distance, MSB-first bit(), 2^120 tolerance; canonical from_hash/to_hash vs raw from_wire/to_wire - the dword-byte-reversal landmine). mule_files::nodes_dat byte-compat reader/writer. Interop test parses the REAL 6098-byte nodes.dat (179 v2 contacts) + round-trips BYTE-IDENTICAL. Research: docs/raw/wave6-kad-research-2026-07-14.md. REMAINING 6a: routing bin-tree + k-buckets (K=10, ALPHA=3). |
| 6b | Kad UDP framing + obfuscation (5b) + bootstrap/hello | - | not started |
