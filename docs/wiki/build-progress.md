...
| 8 | `ios/padMule` SwiftUI shell + lifecycle + sideload | - | not started (needs an Apple toolchain - the 2011/2012 Mac mini via OpenCore Legacy Patcher, or a newer/cloud Mac). Imports the generated `mule_ffi.swift` + links the staticlib. Must render the honest status notice + per-transfer Paused badges + Reconnecting banner, and wire ScenePhase -> MuleEngine.pause()/resume() ([[lifecycle-and-reactivation]]). |
