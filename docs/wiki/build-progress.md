...
| 5d | crypto wiring + gating; secure-ident state machine; differential gates | (implemented directly) | MOSTLY DONE + VALIDATED vs amuled. (1) TCP obfuscation wired into peer connect/accept + validated (download). (2) SecureIdentSession live mutual RSA exchange - padMule VERIFIED real amuled's signature ("peer PASSED secure identification") and completed its half. Caught the on-wire pubkey = SPKI not PKCS#1 (2nd DER spec error). Remaining: credit tie-in (verified ident -> IP binding + wait-time), server-connect obfuscation, MISCOPTIONS2 crypt-bit gating on outbound, secure-ident wired into the transfer session, amuled uploader-pull. |
