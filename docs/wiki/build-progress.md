...
| 8p | OP_OFFERFILES (announce shares to the server) | [[ed2k-server-oracle]] | **DONE (2026-07-19), commit 0239a36.** build_offer_files (server_messages.rs) writes the eD2k offered-file packet (LowID complete file -> FILE_COMPLETE_ID/PORT; HighID -> real id+port; FT_FILENAME+FT_FILESIZE tags), faithful to aMule CreateOfferedFilePacket. ServerLink::offer_files; the engine offers its whole shared library right after login (offer_shared_to; id used locally, never emitted). VALIDATED vs real eserver: it accepted+indexed the file ("1 files, 1 complete, 2 keyw") - making the eserver a DETERMINISTIC search oracle. CLI offer-hold/offer-search. Follow-up: re-offer on a live share change. 413 tests. |
