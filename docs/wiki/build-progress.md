...
| 5c | secure identification (RSA) | - | not started |
| 5d | crypto wiring + gating; obfuscated differential gate | (partial) | PARTIAL: peer connect/accept obfuscation wired + validated vs amuled (download). Remaining: server-connect obfuscation, MISCOPTIONS2 crypt-bit gating on outbound, and the amuled UPLOADER-pull (obfuscation now unblocks it). |
