...
| 8j | verified-identity badge goes LIVE | [[emule-070b-features]] | **DONE (2026-07-19).** fetch_one runs mutual secure-ident with a delivering source (warm connection, gated on advertised sec_ident + bytes>0, 8s best-effort) -> note_source_verified -> the per-source blue seal. Identity now Clone; download_file takes Option<Arc<Identity>>. Does NOT touch the differential path. Proven by a mock-secure-ident unit test. |
