...
| 8 | `ios/padMule` SwiftUI shell + lifecycle + sideload | (implemented directly) | **SKELETON + CI DONE (2026-07-16), never compiled yet.** Path C from [[mac-toolchain-setup]] (the 2011 mini cannot run the Xcode iPadOS 26 needs, but padMule is sideload-only so CI can build UNSIGNED and AltStore re-signs). `ios/project.yml` = XcodeGen spec (pbxproj generated in CI, never committed; wires the uniffi module.modulemap + `-lmule_ffi`; iPad-only; deployment target iOS 16 so an older-SDK build still installs on iPadOS 26). `ios/padMule/Sources`: PadMuleApp (ScenePhase -> pause/resume, only on `.background`), EngineModel (every blocking FFI call off the main thread; polls `drainEvents()`), ContentView (renders all three HARD [[lifecycle-and-reactivation]] requirements: honest foreground-only notice, Reconnecting banner, per-transfer Paused badges). `.github/workflows/ios-build.yml`: rustup+aarch64-apple-ios -> staticlib; bindings from the HOST cdylib (arch-independent); xcodegen; xcodebuild unsigned; zip Payload -> `padMule.ipa` artifact. NO Apple secrets needed. **BLOCKED on: pushing the repo to GitHub (no remote exists) + the first CI run - the Swift/xcodegen wiring has NEVER been compiled (no macOS here), so expect iteration.** |
