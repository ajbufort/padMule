# Feature: Server Hunter - ARCHIVE (shipped narrative + pre-build design)

Updated: 2026-08-04 (split verbatim out of [[feature-server-hunter]], which had
grown past the ~150-line entry guidance - the same treatment build-progress got
when [[build-history]] was carved out on 2026-08-01).

Two things live here, both VERBATIM and neither trimmed: the per-part narrative
of what shipped, and the PRE-BUILD design rationale that preceded it. The
feature is complete and device-verified, so both are history.

What deliberately did NOT move: the scope boundary - why literal whole-net
scanning stays out - is a STANDING decision, not a completed narrative, so it
remains in the live entry where someone proposing a scanner will actually read
it.
...
- [[feature-server-hunter]] - the live entry: status + the scope boundary.
- [[build-progress]] rows 8az / 8ba - the same work in the build ledger.
