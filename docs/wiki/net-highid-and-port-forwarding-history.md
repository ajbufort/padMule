# HighID and Port Forwarding - ARCHIVE

Updated: 2026-08-04 (split verbatim out of [[net-highid-and-port-forwarding]],
which had grown past the ~150-line entry guidance - the same treatment
build-progress got when [[build-history]] was carved out on 2026-08-01). Moved,
not rewritten.

The pre-VPN era: the old Xfinity five-link chain, its live validation, the
iPad's first HighID over unicast SSDP, and the stale-mapping dead end.

Superseded by the BE9700 topology and then by AirVPN, but kept verbatim for two
reasons. The stale-mapping episode is WHY delete-then-add exists, so deleting it
would invite the bug back. And the five-link chain is still the right mental
model for anyone debugging a plain home router - which is every user who is not
on a VPN.
...
## VPN vendor survey + the port blocker it exposed (2026-08-03)

The research that led to AirVPN, and the padMule blocker it surfaced. Archived
because both are RESOLVED: the port override shipped (8bd) and the vendor choice
is made. The general facts stay useful if the provider ever changes.
...
- [[net-highid-and-port-forwarding]] - the live entry (the AirVPN reality).
- [[padmule-dev-box-networking]] / [[build-progress]]
